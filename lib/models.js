...
DataModel.prototype.save = function (key) {
  if (!this.backend) {
    throw new Error("you must specify a backend");
  }
  return this.backend.save(key, this);
};

DataModel.prototype.load = function (key) {
  if (!this.backend) {
    throw new Error("you must specify a backend");
  }
  return this.backend.load(key, this);
};
...
DataModel.prototype.valueOf = function () {
  return utils.map(this.data, function (value, field) {
...
function createModel(options) {
  ...
  function Model(opt_data) {
    ...
    utils.each(this.schema, function (field, fieldName) {
      if (utils.has(data, fieldName)) {
        if (field && isDataEmitter(field.prototype) && !data[fieldName].isPrototypeOf(field)) {
          this[fieldName] = new (this.schema[fieldName])(data[fieldName]);
          return;
        }
        this[fieldName] = data[fieldName];
        return;
      }
      if (utils.has(defaults, fieldName)) {
      ...
      this[fieldName] = field.defaults_;
    }, this);
    this.silent_ = false;
  ...
  utils.each(fieldTypes, function (fieldType, fieldName) {
    ...
    Object.defineProperty(Model.prototype, fieldName, {
      ...
      set: function (v) {
        var err, current = this.data[fieldName];
        ...
        if (isDataEmitter(v)) {
          this.eventIds[fieldName] = v.on(function (name) {
            if (v.replaceEventName) {
              name = name.replace(fieldRegex, fieldName);
            } else {
              name = fieldName + "." + name;
            }
            this.update(name);
