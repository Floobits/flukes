...
function createActions (actions) {
  ...
  function Action_ () {
    this.fields_ = Object.keys(actions).map(function(f) {return f.toUpperCase();});
    ...
    utils.each(actions, function (theAction, actionName) {
      var constant = actionName.toUpperCase();
      if (!utils.isFunction(theAction)) {
        this[actionName] = theAction;
        return;
      }
      this["on" + constant] = this.on.bind(this, constant);
      if (actionName.indexOf("async") !== 0 ) {
        this[actionName] = function action_action () {
          ...
          this.emit.apply(this, [constant].concat(ret));
          return null;
        }.bind(this);
        return;
      }
      this[actionName] = function action_action () {
        ...
        this.emitAsync.call(this, cb.bind(that, ret), constant, ret);
        return null;
      }.bind(this);
      return;
    }, this);
